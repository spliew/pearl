...
Except function calculate_kid_given_data(), all the other functions are from https://github.com/abdulfatir/gan-metrics-pytorch
"""
import os
import pathlib
import sys
...
def get_activations(files, model, batch_size=50, dims=2048,
    ...
    -- A numpy array of dimension (num images, dims) that contains the
       activations of the given tensor whe feeding inception with the
       query tensor.
    """
    model.eval()
...
def calculate_kid_given_data(rec_imgs, real_imgs, batch_size, cuda, dims, model_type='inception'):
    """Calculates the KID of two paths"""
    ...
    act_true = _compute_activations(real_imgs, model, batch_size, dims, cuda, model_type)
    results = []
    actj = _compute_activations(rec_imgs, model, batch_size, dims, cuda, model_type)
    kid_values = polynomial_mmd_averages(act_true, actj, n_subsets=100)
