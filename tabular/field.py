...
class NumericalField:
    ...
    def learn(self):
        ...
        if self.model_name == "gmm":
            # if len(self.fitdata.shape) = 1:
            #     self.fitdata = self.fitdata.to_numpy().reshape(-1, 1)
            ...
            self.K = 2/(self.Max-self.Min + 1e-16)
...
# Field for categorical column
    ...
    def __init__(self, method="one-hot",noise=0.2, embed_dim = 3):
        self.dict = {}
        self.rev_dict = {}
        self.method = method
        self.fitdata = None
    ...
    def learn(self):
        ...
        if self.method == 'embed':
            self.emb = nn.Embedding(vset.shape[0], self.embed_dim)
    ...
    def convert(self, data):
        ...
        if self.method == "dict":
            return features
        if self.method == 'embed':
            features = torch.from_numpy(features.squeeze(1))
            features = self.emb(features).detach().numpy()
            return features
    ...
    def reverse(self, features):
        ...
        if self.method == 'embed':
            ...
            for i in range(rownum):
                feat.append(torch.argmax(nn.functional.cosine_similarity(self.emb.weight.data,features[i,:].reshape(1,-1))).numpy())
